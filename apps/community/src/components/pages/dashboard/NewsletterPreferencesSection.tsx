...
const frequencyOptions: Array<{ value: NewsletterDigestFrequency; label: string }> = [
...
export function NewsletterPreferencesSection({
  ...
  const [frequency, setFrequency] = useState<NewsletterDigestFrequency>(initialSettings.frequency);
  const [includeBlog, setIncludeBlog] = useState(initialSettings.includeBlog);
  const [includeDiscussions, setIncludeDiscussions] = useState(initialSettings.includeDiscussions);
  const [search, setSearch] = useState("");
  const [includedFollowingIds, setIncludedFollowingIds] = useState<Set<string>>(
    () => new Set(followingUsers.map((user) => user.id).filter((id) => !initialSettings.mutedFollowingIds.includes(id)))
  );
  ...
  const filteredFollowing = useMemo(() => {
    ...
    return followingUsers.filter((user) => {
      return user.name.toLowerCase().includes(phrase) || user.username.toLowerCase().includes(phrase);
    });
  ...
  const handleSaveSettings = async () => {
    ...
    if (!result.success) {
      setMessage({ type: "error", text: result.error ?? "Failed to save newsletter settings." });
      return;
    }
  ...
  const handleSaveFollows = async () => {
    ...
    if (!result.success) {
      setMessage({ type: "error", text: result.error ?? "Failed to save followed creator preferences." });
      return;
    }
    setMessage({ type: "success", text: "Followed creator preferences saved." });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div>
          <h3 className="text-sm font-semibold text-foreground">Newsletter preferences</h3>
          <p className="mt-1 text-xs text-muted-foreground">
            Configure digest frequency and choose which followed creators are included.
          </p>
        </div>
        <ChevronDown
      ...
      {isOpen ? (
        <div id="newsletter-preferences-content" className="space-y-5">
          <div className="grid gap-4 md:grid-cols-2">
            <label className="space-y-1">
              <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Frequency</span>
              <select
                value={frequency}
                onChange={(event) => setFrequency(event.target.value as NewsletterDigestFrequency)}
            ...
            <div className="space-y-2 rounded-md border border-border/70 bg-background/60 p-3">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Include in digest</p>
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(event) => setIncludeDiscussions(event.target.checked)}
                  disabled={isSavingSettings}
                />
                Discussion posts
              </label>
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Followed creators</p>
            ...
            {followingUsers.length > 0 ? (
              <>
                ...
                <div className="max-h-72 space-y-2 overflow-y-auto rounded-md border border-border/70 bg-background/60 p-3">
                  ...
                  {filteredFollowing.length === 0 ? (
                    <p className="text-xs text-muted-foreground">No followed creators match your search.</p>
                  ) : null}
                </div>
                <button
                  type="button"
              ...
              <p className="text-xs text-muted-foreground">
                You are not following anyone yet. Follow creators to include their updates in your digest.
              </p>
            )}
          </div>

          {message ? (
            <p className={`text-xs ${message.type === "success" ? "text-emerald-600" : "text-destructive"}`} role="status">
              {message.text}
            </p>
          ) : null}
        </div>
